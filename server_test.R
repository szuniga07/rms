...
## Modify the variable type of data   
    ...
    modifiedCharFnc <- function(df, ModifyCharacter) {
      ...
      return(df_mod[, which(colnames(df_mod) %in% ModifyCharacter)])
    }
    #Runs the function above
    ...
    modifiedFacFnc <- function(df, ModifyFactor) {
      ...
      return(df_mod[, which(colnames(df_mod) %in% ModifyFactor)])
    }
    #Runs the function above
    ...
    modifiedNumFnc <- function(df, ModifyNumeric) {
      df_mod <- df
        ...
        df_mod[, which(colnames(df_mod) %in% ModifyNumeric)] <- sapply(df_mod[which(colnames(df_mod) %in% ModifyNumeric)], as.numeric)
      }
      return(df_mod[, which(colnames(df_mod) %in% ModifyNumeric)])
    }
    #Runs the function above
    ...
    output$modified_df_name <- renderUI({ 
      textInput("ModifiedDfName", "8. Enter the DFBETAS name.", 
                value= "mod_df")     
    })
